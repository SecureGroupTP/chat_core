...
Экспортируемый `serialized_client_state` теперь сохраняет не только runtime-состояние уровня
приложения, но и сериализуемый dump `OpenMLS` provider storage.
Это позволяет `restore_client` после `export_client_state` полноценно восстанавливать:

- идентичность клиента
- инвентарь и криптографическое состояние key package
- локальные MLS-группы, включая их epoch/pending storage state

После `export -> restore` клиент может продолжать рабочий `encrypt/decrypt` flow в уже
существующих группах без повторного join.

Важно: старые snapshots без backend dump по-прежнему совместимы только для случаев без групп.
Если в таком снимке есть `groups`, `restore_client` вернёт `Unsupported`, потому что без
backend storage это было бы частичное и небезопасное восстановление OpenMLS-состояния.
